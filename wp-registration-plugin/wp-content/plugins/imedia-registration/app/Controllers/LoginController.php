...
final readonly class LoginController
{
    ...
    public function login(Request $req, Response $res): Response
    {
        ...
        if (LoginAttempt::recentFailuresByIp($ip) >= LoginAttempt::IP_MAX_FAILURES) {
            Logger::warning('auth.throttle.ip', ['ip' => $ip, 'email' => $email]);
        ...
        if ($existing !== null && !empty($existing['locked_until'])) {
            ...
            if ($until !== false && $until > time()) {
                Logger::warning('auth.throttle.locked', ['admin_id' => $existing['id'], 'email' => $email]);
        ...
        if ($admin === null) {
            ...
            if ($emailFails >= LoginAttempt::EMAIL_MAX_FAILURES && $existing !== null) {
                ...
                Logger::warning('auth.throttle.lockout', ['admin_id' => $existing['id'], 'email' => $email]);
            } else {
                Logger::info('auth.login.failed', ['email' => $email, 'ip' => $ip]);
            }
