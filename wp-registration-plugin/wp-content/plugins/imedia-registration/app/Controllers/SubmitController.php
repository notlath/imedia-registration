...
final readonly class SubmitController
{
    ...
    private const APPLICATION_FIELDS = [
        'name', 'mobile', 'email', 'position', 'message',
    ];
    ...
    public function handle(Request $req, Response $res): Response
    {
        ...
        if ($route === null) {
            Logger::warning('submit.no_route', ['form_id' => $formId, 'ip' => $req->ip()]);
            return $this->error(
                $res,
                404,
                'no_route',
        ...
        if ($route['target_type'] === FormRoute::TARGET_REGISTRATION) {
            try {
                ...
                Logger::error('threshold.check_failed', [
    ...
    private function insertRegistration(array $fields): int
    {
        ...
        $id = Registration::insert($typed + ['dynamic_data' => $dynamic]);
    ...
    private function insertContact(array $fields): int
    {
        ...
        $id = Contact::insert($typed + ['dynamic_data' => $dynamic]);
        StatusLogger::log(
    ...
    private function insertApplication(array $fields, string $type): int
    {
        ...
        $id = Application::insert($typed + ['type' => $type, 'dynamic_data' => $dynamic]);
        StatusLogger::log(
     ...
     */
    private function error(Response $res, int $status, string $code, string $message, array $extra = []): Response
    {
        return $res->json(array_merge([
            'success' => false,
            'error'   => $code,
            'message' => $message,
        ], $extra), $status);
