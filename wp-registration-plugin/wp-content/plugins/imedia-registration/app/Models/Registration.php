...
{
    public const STATUSES      = ['pending', 'tentative', 'confirm', 'forfeit', 'reschedule'];
    public const PAYMENT_STATUSES = ['pending', 'deposit', 'fully_paid'];
    ...
    public static function countConfirmedForSlot(string $course, int $year, int $month): int
    {
        ...
        $stmt->execute([':c' => $course, ':d1' => $d1, ':d2' => $d2]);
    ...
    public static function stats(): array
    {
        ...
        $stmt->execute([':today' => $today, ':week' => $week, ':month' => $month]);
        $row = $stmt->fetch() ?: [];
     ...
     */
    public static function allForExport(array $filters = []): \Generator
    {
        $where = ['deleted_at IS NULL'];
        $bind  = [];
        if (!empty($filters['status']) && in_array($filters['status'], self::STATUSES, true)) {
    ...
    public static function update(int $id, array $data): array
    {
        $before = self::find($id);
        if ($before === null) {
            return ['before' => null, 'after' => null, 'changed' => false, 'prev_status' => null, 'new_status' => null];
        }

        $set = [];
        $bind = [':id' => $id];
        $editable = [
            'name', 'mobile', 'email', 'address', 'course',
            'start_date', 'end_date', 'status', 'payment_status',
            'paid_amount', 'paid_at', 'remark',
        ];
        ...
        if ($set === []) {
            return ['before' => $before, 'after' => $before, 'changed' => false, 'prev_status' => $before['status'], 'new_status' => $before['status']];
        }
    ...
    public static function setResumePath(int $id, string $relativePath): void
    {
        ...
        $stmt->execute([':p' => $relativePath, ':id' => $id]);
    }
