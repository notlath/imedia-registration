...
class IMF_Frontend
{
    ...
    {
        add_shortcode('imedia_form', [$this, 'render_shortcode']);
        add_action('wp_enqueue_scripts', [$this, 'enqueue_assets']);
    }

    public function enqueue_assets()
    {
        wp_enqueue_style('imf-frontend-css', Imedia_Forms::plugin_url() . 'resources/assets/css/style.css', [], Imedia_Forms::VERSION);
        wp_enqueue_script('imf-frontend-js', Imedia_Forms::plugin_url() . 'resources/assets/js/script.js', [], Imedia_Forms::VERSION, true);
    }

    public function render_shortcode($atts)
    {
        $atts = shortcode_atts(['id' => 0, 'category' => ''], $atts);
        $id = intval($atts['id']);
        if (! $id) return '<p>Invalid form ID.</p>';

        $post = get_post($id);
        if (! $post || $post->post_type !== Imedia_Forms::CPT) return '<p>Form not found.</p>';
        ...
        if (! is_array($fields) || empty($fields)) return '<p>This form has no fields.</p>';
...
<?php
    ...
    private function render_frontend_field($field, $category = '')
    {
        ...
        $data_attrs = '';
        if (!empty($field['required'])) $data_attrs .= ' data-required="1"';
        if (!empty($v['min_length'])) $data_attrs .= ' data-minlength="' . esc_attr($v['min_length']) . '"';
        if (!empty($v['max_length'])) $data_attrs .= ' data-maxlength="' . esc_attr($v['max_length']) . '"';
        if (!empty($v['pattern'])) $data_attrs .= ' data-pattern="' . esc_attr($v['pattern']) . '"';
        if (!empty($v['custom_error'])) $data_attrs .= ' data-error="' . esc_attr($v['custom_error']) . '"';

        $html = '<div class="imf-field imf-field-w' . esc_attr($w) . $cc . '"' . $data_attrs . '>';

        if ($t === 'section') {
            $html .= '<div class="imf-section-divider"><h4>' . $l . '</h4>';
            if (!empty($field['default_value'])) $html .= '<p>' . esc_html($field['default_value']) . '</p>';
            $html .= '</div></div>';
            return $html;
        }
        ...
        switch ($t) {
            ...
            case 'multiselect':
                ...
                if ($t !== 'multiselect') $html .= '<option value="">' . esc_html($field['placeholder'] ?? 'Please select') . '</option>';
                $inGroup = false;
                $currentCategory = '';
                foreach ($opts as $o) {
                    $o = trim($o);
