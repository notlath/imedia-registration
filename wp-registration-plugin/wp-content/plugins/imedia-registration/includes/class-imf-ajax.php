...
class IMF_Ajax {
    public function __construct() {
        // Form AJAX handlers
        add_action('wp_ajax_imf_toggle_status', [$this, 'ajax_toggle_status']);
        add_action('wp_ajax_imf_delete_form', [$this, 'ajax_delete_form']);
        add_action('wp_ajax_imf_create_form', [$this, 'ajax_create_form']);

        // Entry AJAX handlers
        add_action('wp_ajax_imf_delete_entry', [$this, 'ajax_delete_entry']);
        add_action('wp_ajax_imf_bulk_entry_action', [$this, 'ajax_bulk_entry_action']);
        add_action('wp_ajax_imf_toggle_star', [$this, 'ajax_toggle_star']);
        add_action('wp_ajax_imf_toggle_read', [$this, 'ajax_toggle_read']);
        add_action('wp_ajax_imf_save_column_prefs', [$this, 'ajax_save_column_prefs']);

        // Export handler
        add_action('admin_post_imf_export_entries_csv', [$this, 'export_entries_csv']);
    }
    ...
    public function ajax_toggle_status() {
        ...
        if (! $form_id || ! current_user_can('edit_post', $form_id)) wp_send_json_error();
    ...
    public function ajax_delete_form() {
        check_ajax_referer('imf_nonce', 'nonce');
        $form_id = intval($_POST['form_id']);
        if (! $form_id || ! current_user_can('delete_post', $form_id)) wp_send_json_error();
    ...
    public function ajax_delete_entry() {
        ...
        $wpdb->update(IMF_Database::table_name(), ['status' => 'trash'], ['id' => $id]);
        wp_send_json_success();
    }
    ...
    public function ajax_toggle_star() {
        ...
        $wpdb->update($table, ['is_starred' => $current ? 0 : 1], ['id' => $id]);
        wp_send_json_success(['is_starred' => !$current]);
    }
    ...
    public function ajax_toggle_read() {
        ...
        wp_send_json_success(['is_read' => !$current]);
    }
    ...
    public function export_entries_csv() {
        ...
        $header = ['Entry ID', 'Date', 'IP Address', 'User Agent'];
        ...
        header('Content-Disposition: attachment; filename=form-' . $form_id . '-entries-' . date('Y-m-d') . '.csv');
