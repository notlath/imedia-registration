...
class IMF_Admin {
    public function __construct() {
        ...
        add_action('save_post_' . Imedia_Forms::CPT, [$this, 'save_meta'], 10, 2);
    ...
    public function enqueue_admin_assets($hook) {
        ...
        if (in_array($current_page, $dashboard_pages)) {
            wp_enqueue_style('imf-admin-css', plugin_dir_url(dirname(__FILE__)) . 'resources/assets/css/form-builder.css', [], Imedia_Forms::VERSION);
            wp_enqueue_script('imf-dashboard-js', plugin_dir_url(dirname(__FILE__)) . 'resources/assets/js/dashboard.js', [], Imedia_Forms::VERSION, true);
            wp_localize_script('imf-dashboard-js', 'imfDashboard', [
            ...
            if (in_array($current_page, ['imedia-forms-entries', 'imedia-forms-all-entries', 'imedia-forms-entry-detail'])) {
                wp_enqueue_style('imf-entries-css', plugin_dir_url(dirname(__FILE__)) . 'resources/assets/css/entries.css', ['imf-admin-css'], Imedia_Forms::VERSION);
                wp_enqueue_script('imf-entries-js', plugin_dir_url(dirname(__FILE__)) . 'resources/assets/js/entries.js', ['imf-dashboard-js'], Imedia_Forms::VERSION, true);
            }
        }

        // Builder page
        if ($screen && $screen->post_type === Imedia_Forms::CPT && in_array($hook, ['post.php', 'post-new.php'])) {
            wp_enqueue_style('imf-builder-css', plugin_dir_url(dirname(__FILE__)) . 'resources/assets/css/form-builder.css', [], Imedia_Forms::VERSION);
            wp_enqueue_script('sortablejs', 'https://cdn.jsdelivr.net/npm/sortablejs@1.15.0/Sortable.min.js', [], '1.15.0', true);
            wp_enqueue_script('imf-builder-js', plugin_dir_url(dirname(__FILE__)) . 'resources/assets/js/form-builder.js', ['sortablejs'], Imedia_Forms::VERSION, true);
    ...
    public function render_dashboard() {
        ...
        foreach ($forms as $form) {
            ...
            if ($st === 'inactive') $count_inactive++;
            else $count_active++;
        }

        if ($status_filter === 'active') {
            $forms = array_filter($forms, function ($f) {
        ...
        include plugin_dir_path(dirname(__FILE__)) . 'resources/views/wordpress/dashboard.php';
    }
    ...
    public function render_settings_page() {
        ...
        include plugin_dir_path(dirname(__FILE__)) . 'resources/views/wordpress/settings.php';
    }
    ...
    public function render_builder($post) {
        ...
        include plugin_dir_path(dirname(__FILE__)) . 'resources/views/wordpress/builder.php';
    }
    ...
    public function save_meta($post_id, $post) {
        if (! isset($_POST['imf_meta_nonce']) || ! wp_verify_nonce($_POST['imf_meta_nonce'], 'imf_save_meta')) return;
        ...
        if (isset($_POST['imf_appearance'])) {
            ...
            if (is_array($appearance)) {
                $safe = [
                    ...
                    'submit_width'      => in_array($appearance['submit_width'] ?? '', ['auto', 'full']) ? $appearance['submit_width'] : 'auto',
                    'submit_alignment'  => in_array($appearance['submit_alignment'] ?? '', ['left', 'center', 'right']) ? $appearance['submit_alignment'] : 'left',
    ...
    public function render_entries_page() {
        ...
        $fields = json_decode(get_post_meta($form_id, Imedia_Forms::META_FIELDS, true), true) ?: [];
        $all_fields = [];
        foreach ($fields as $f) {
            if (in_array($f['type'], ['section', 'hidden'])) continue;
            $all_fields[] = $f;
        }

        $pref_key = '_imf_columns_' . $form_id;
        $user_prefs = get_user_meta(get_current_user_id(), $pref_key, true);
        $visible_columns = is_array($user_prefs) ? $user_prefs : [];

        if (empty($user_prefs) && !empty($all_fields)) {
            $visible_columns = array_slice(array_map(function ($f) {
                return $f['name'] ?? $f['id'];
            }, $all_fields), 0, 4);
        }
        ...
        include plugin_dir_path(dirname(__FILE__)) . 'resources/views/wordpress/entries-list.php';
    }
    ...
    public function render_entry_detail_page() {
        ...
        if (!$entry->is_read) {
            $wpdb->update($table, ['is_read' => 1], ['id' => $entry_id]);
        }
        ...
        include plugin_dir_path(dirname(__FILE__)) . 'resources/views/wordpress/entry-detail.php';
    }
    ...
    public function render_all_entries_page() {
        ...
        include plugin_dir_path(dirname(__FILE__)) . 'resources/views/wordpress/all-entries.php';
    }
}
