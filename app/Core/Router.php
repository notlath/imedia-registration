...
final class Router {
     ...
     */
    public function add( string $method, string $path, callable|array $handler, array $middleware = array() ): void {
        $method = strtoupper($method);
        $this->routes[ $method ][] = array( $path, $handler, $middleware );
    }

    public function get( string $path, callable|array $handler, array $middleware = array() ): void {
        $this->add('GET', $path, $handler, $middleware);
    }

    public function post( string $path, callable|array $handler, array $middleware = array() ): void {
    ...
    public function dispatch( Request $req, Response $res ): Response {
        ...
        $candidates = $this->routes[ $method ] ?? array();
    ...
    private function runChain( Request $req, Response $res, callable|array $handler, array $middleware, int $index ): Response {
        ...
        return $instance(
            $req,
            $res,
            function ( Request $r, Response $rr ) use ( $handler, $middleware, $index ): Response {
