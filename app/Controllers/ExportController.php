...
final readonly class ExportController {
    public function stream( Request $req, Response $res ): Response {
        ...
        // Fallback: extract from the last path segment if param not set
        if ($type === '') {
            $segments = explode('/', trim($req->path, '/'));
            $last = (string) end($segments);
            $type = str_replace('.csv', '', $last);
        }
        $date = date('Y-m-d');
        ...
        foreach (Registration::allForExport() as $row) {
            fputcsv(
                ...
                array(
                    ...
                    $row['paid_at'] ?? '',
                    $row['remark'] ?? '',
                    json_encode($row['dynamic_data'] ?? array(), JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE),
                    $row['created_at'],
                    $row['updated_at'],
                )
            );
    ...
    private function streamContacts( Response $res, string $date ): Response {
        ...
        fputcsv($out, array( 'id', 'name', 'mobile', 'email', 'subject', 'message', 'status', 'remarks', 'created_at' ));
        Database::unbuffered(
            function () use ( $out ) {
    ...
    private function streamApplications( Response $res, string $date, string $type ): Response {
        ...
        fputcsv($out, array( 'id', 'type', 'name', 'mobile', 'email', 'position', 'message', 'resume_filename', 'status', 'remarks', 'created_at' ));
        Database::unbuffered(
            function () use ( $out, $type ) {
