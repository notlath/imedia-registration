...
final readonly class RegistrationsController {
    ...
    public function create( Request $req, Response $res ): Response {
        ...
        $id = Registration::insert($input + array( 'dynamic_data' => array() ));
        StatusLogger::log(StatusHistory::ENTITY_REGISTRATION, $id, StatusHistory::FIELD_STATUS, null, $input['status'] ?? 'pending', null, 'Created via admin');
        try {
    ...
    public function update( Request $req, Response $res ): Response {
        ...
        if (isset($input['dynamic_data']) && is_string($input['dynamic_data'])) {
            ...
            $input['dynamic_data'] = is_array($decoded) ? $decoded : array();
        }
        ...
        if ($result['changed']) {
            $before = $result['before'] ?? array();
            $after  = $result['after'] ?? array();
    ...
    public function bulkStatus( Request $req, Response $res ): Response {
        $ids = $req->input('ids', array());
        $newStatus = (string) $req->input('new_status', '');
        if (! is_array($ids) || $ids === array() || ! in_array($newStatus, Registration::STATUSES, true)) {
    ...
    private static function handleResumeUpload( Request $req, int $registrationId ): ?array {
        ...
        try {
            ...
            \App\Core\Logger::warning(
                'registration.resume_upload_failed',
                array(
    ...
    private static function readInput( Request $req ): array {
        $input = (array) $req->input('reg', array());
