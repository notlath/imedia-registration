...
final class HmacVerify {
    ...
    public function __invoke( Request $req, Response $res, callable $next ): Response {
        ...
        if (! is_string($header) || $header === '') {
            ...
            return $res->json(
                array(
                    'success' => false,
                    'error' => 'hmac_missing',
                    'message' => 'Missing X-IMF-Signature header.',
                ),
                401
            );
        }
        ...
        if (! is_string($secret) || $secret === '') {
            Logger::error('hmac.secret_not_configured', array( 'ip' => $req->ip() ));
            return $res->json(
                array(
                    'success' => false,
        ...
        if (! is_int($tsField) && ! ( is_string($tsField) && ctype_digit($tsField) )) {
            ...
            return $res->json(
                array(
                    'success' => false,
                    'error' => 'hmac_timestamp_missing',
                    'message' => 'Missing or invalid _imf_timestamp.',
                ),
                401
            );
        }
        ...
        if (abs($now - $ts) > self::MAX_SKEW_SECONDS) {
            ...
            return $res->json(
                array(
                    'success' => false,
                    'error' => 'hmac_timestamp_stale',
                    'message' => 'Request timestamp is outside the allowed freshness window.',
                ),
                401
            );
        }
